...
# detailed difference in one file
def get_file_result(path):
    new_html = os.path.join(args.new, 'coverage', 'src', 'PROJ', f'{path}.html')
    old_html = os.path.join(args.old, 'coverage', 'src', 'PROJ', f'{path}.html')
    with open(os.path.join(old_html), "r") as f:
        old_html = BeautifulSoup(f.read(), features="html.parser")

    with open(os.path.join(new_html), "r") as f:
...
# file_details = {}
# for file in files:
#     file_details[file] = get_file_result(file)
